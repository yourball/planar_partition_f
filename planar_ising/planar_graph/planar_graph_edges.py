...
class PlanarGraphEdges:
    ...
    def _init_graph_from_database(self, database):

        for idx, entry in enumerate(database):
            self._vertex1[idx], self._vertex2[idx] = entry[1], entry[2]
            self._vertex1_next_edge_index[idx] = entry[3]
            self._vertex1_previous_edge_index[idx] = entry[4]
            self._vertex2_next_edge_index[idx] = entry[5]
            self._vertex2_previous_edge_index[idx] = entry[6]
