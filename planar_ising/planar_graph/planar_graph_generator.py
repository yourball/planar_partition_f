...
class PlanarGraphGenerator:
    ...
    @staticmethod
    def generate_triang_lattice_graph(database):
        capacity = len(database)
        edges = PlanarGraphEdges(capacity)
        edges._init_graph_from_database(database)

        vertex_set = np.concatenate([edges._vertex1, edges._vertex2])
        # Total number of vertexes in the graph
        vertex_num = np.max(vertex_set) + 1
        vertex_costs = np.ones(vertex_num)
        vertex_costs /= np.sum(vertex_costs)

        vertex_seen = []
        incident_edge_indices = []
        # indx, v1, v2, c1, cc1, c2, cc2
        for idx, entry in enumerate(database):
            v1, v2 = entry[1], entry[2]
            # print('v1, v2:', v1, v2)
            edge = idx
            # print('vertex_seen', vertex_seen)
            if v1 not in vertex_seen:
                # print('v1 is not seen:', v1)
                vertex_seen.append(v1)
                incident_edge_indices.append(edge)
                # print('edge:', edge)
            elif v2 not in vertex_seen:
                # print('v2 is not seen:', v2)
                vertex_seen.append(v2)
                incident_edge_indices.append(edge)
                # print('edge:', edge)

        incident_edge_indices = np.array(incident_edge_indices)
        incident_edge_indices = incident_edge_indices[np.argsort(vertex_seen)]
        # print('incident_edge_indices', incident_edge_indices)

        graph = PlanarGraph(vertex_costs, incident_edge_indices, edges)
        return graph
